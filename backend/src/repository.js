...
/**
 * Of the slugs currently on screen, the ones worth spending an OMDb request on.
 *
 * Selection is demand-driven rather than a catalog walk: the caller passes the
 * slugs the home/list viewmodels just rendered, and the result keeps that exact
 * order, so a run truncated by the daily budget still covers the most visible
 * rows first. `array_position` is what preserves that ordering across the join.
 *
 * Rows already known to be unreachable ('no-imdb-id'/'no-tmdb-id') are excluded
 * outright; matched and unmatched rows come back only once their respective TTL
 * has elapsed, a miss being retried far more slowly because OMDb having no
 * entry for a title rarely changes.
 */
export async function listOmdbRatingCandidates(slugs = [], limit = config.omdbBatchLimit) {
  const ordered = [...new Set(slugs.filter(Boolean).map(String))];
  if (!ordered.length) return [];
  const result = await pool.query(
    'SELECT id, canonical_slug, title, original_title, media_type, ' +
    'imdb_id, omdb_imdb_id, tmdb_id, tmdb_media_type, omdb_status FROM movies ' +
    "WHERE canonical_slug = ANY($1::text[]) AND catalog_state = 'ready' " +
    "AND omdb_status NOT IN ('no-imdb-id', 'no-tmdb-id') " +
    'AND (omdb_checked_at IS NULL ' +
    "  OR (omdb_status = 'matched' AND omdb_checked_at < now() - ($2::bigint * interval '1 millisecond')) " +
    "  OR (omdb_status <> 'matched' AND omdb_checked_at < now() - ($3::bigint * interval '1 millisecond'))) " +
    'ORDER BY array_position($1::text[], canonical_slug) LIMIT $4',
    [ordered, config.omdbRefreshMs, config.omdbMissRetryMs, Math.max(1, Math.floor(limit))]
  );
  return result.rows;
}

export async function recordOmdbScores(movieId, scores) {
  const result = await pool.query(
    "UPDATE movies SET omdb_status='matched', omdb_imdb_id=$2, omdb_tomatometer=$3, " +
    'omdb_metascore=$4, omdb_imdb_rating=$5, omdb_checked_at=now(), updated_at=now() ' +
    'WHERE id=$1 RETURNING *',
    [movieId, scores.imdbId, scores.tomatometer, scores.metascore, scores.imdbRating]
  );
  return result.rows[0] || null;
}

/**
 * Record why a row produced no scores. `updated_at` is deliberately left alone
 * for misses: nothing a visitor can see changed, so this must not look like a
 * content change to anything ordering on it.
 */
export async function recordOmdbMiss(movieId, status, imdbId = null) {
  const allowed = ['unmatched', 'no-imdb-id', 'no-tmdb-id', 'error'].includes(status)
    ? status : 'error';
  const result = await pool.query(
    'UPDATE movies SET omdb_status=$2, omdb_imdb_id=COALESCE($3, omdb_imdb_id), ' +
    'omdb_checked_at=now() WHERE id=$1 RETURNING canonical_slug',
    [movieId, allowed, imdbId]
  );
  return result.rows[0] || null;
}
