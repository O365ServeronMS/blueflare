...
- **Provider sync** (`worker`, every `SYNC_INTERVAL_MS`): crawls NguonC/KKPhim head pages plus a checkpointed backfill, upserts canonical rows, then invalidates Valkey keys and Next render tags for exactly what changed.
...
`TMDB_API_KEY` is the one secret `deploy/bootstrap-vps.sh` cannot regenerate; it
has to be kept off the machine.
